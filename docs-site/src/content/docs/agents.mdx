...
Effort metadata does not choose an implementation model. `ctx:S|M|L|XL` estimates session size,
`role:*` records responsibility, and orchestration's `--agent` flag chooses the CLI program. A
launched Codex or Claude session uses that program's configured default model.
