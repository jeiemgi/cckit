...
Effort metadata still does not choose a model: `ctx:S|M|L|XL` estimates session size and `role:*`
records responsibility. The model comes from an **agent profile** — `agents.profiles.<name>.model`
in the config — which also carries the reasoning level, permission policy and extra CLI argv. An
`agent:<profile>` label on the issue (or its parent effort) selects one; `agents.default` applies
when nothing else does. Orchestration's `--agent` flag still names the CLI program directly, and a
session launched that way uses that program's own configured model.
