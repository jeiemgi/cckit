...
Nothing in cckit picks a model — not `ctx:S|M|L|XL` (which estimates session size), not `role:*`
(which records responsibility), and not the config schema. An **agent profile** declares a spend
`tier` and the CLI arguments to run; you spell any model selection yourself in `args`. An
`agent:<profile>` label on the issue (or its parent effort) selects a profile, and with
`agents.default` unset the cheapest-tier profile wins, so the inexpensive path is the default.
Orchestration's `--agent` flag still names a CLI program directly, and that path uses the
program's own configured model.
