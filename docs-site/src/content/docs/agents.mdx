...
| --- | --- | --- |
| Read the board | `cckit sync --llm` | JSON |
| Find the next unblocked issue | `cckit next --llm` | TOON |
| Get the wave plan | `cckit plan --llm` | TOON |
| Get the fan-out brief | `cckit copilot --llm` | TOON |
| Start an issue | `cckit start <issue>` | JSON |
| Open the PR | `cckit pr <issue> "<summary>"` | JSON |
| Close the issue | `cckit close <issue> "<summary>"` | JSON |
| Render markdown to a terminal | `cckit render` | — |

<Combo>[`cckit copilot`](/copilot/) fans a wave out · [`cckit watch`](/copilot/) gates and merges it · [`cckit next`](/github-cycle/) is the by-hand version for one issue.</Combo>
...
<CardGrid>
  <LinkCard title="The copilot loop" href="/copilot/" description="Fan a wave of issues out to parallel agents." />
  <LinkCard title="Adapters" href="/adapters/" description="The agnostic core and the per-agent adapters." />
  <LinkCard title="Config & permissions" href="/config-and-permissions/" description="How cckit finds config and asks before it mutates." />
</CardGrid>
