...
| `cckit cleanup [--yes]` | The guided destructive sweep over the `gc` classification: prints the plan (SAFE / ORPHAN / PROTECTED / stashes) and writes nothing without `--yes`. `--yes` deletes the SAFE worktrees, local branches, and remote refs that were level with their branch. An ORPHAN branch, a branch whose issue is still open, and a stash are never deletable — they are surfaced and left alone. |
