...
<Tag kind="flow" /> <Tag kind="conversational" />

Copy-paste prompts and the exact command flow for each common job, ready to hand an agent.
...
- **The prompt** — copy-paste text you hand the agent. In most cases you prompt; you don't type the commands yourself.
- **The flow** — the correct order of commands the agent runs, so it never skips or invents a step.
...
<Aside type="note" title="How an agent drives cckit">
Every recipe builds on the same pattern: the agent **reads state as data** (`cckit sync --llm`, `cckit scan --llm`, `cckit status`), **acts through verbs** one lifecycle step at a time (`start` → `pr` → `close`), and **never hardcodes** an org or repo — everything resolves from `cckit.config.json`.
</Aside>
...
<Combo>[`cckit next`](/cli-reference/) picks the issue for you · [`cckit copilot`](/copilot/) does this for a whole wave at once.</Combo>
...
## Fan out a wave with the copilot

For a whole wave of unblocked issues at once, the **copilot** writes the brief and the agent fans it out into one subagent per issue. The **captain** (`cckit watch`) gates the resulting PRs and merges the clean ones.

**Prompt:**

```text
Clear the next wave with the cckit copilot. Run `cckit copilot` to get the fan-out brief, spawn
one subagent per issue as it says, then `cckit watch --merge` to gate the open PRs and merge the
clean ones. Re-run `cckit copilot` to advance to the next wave and repeat until nothing is left.
```

**Flow:**

```bash
cckit copilot          # emit the wave-0 subagent fan-out brief
...
<Combo>[`cckit plan`](/efforts-and-waves/) builds the wave → [`cckit copilot`](/copilot/) fans it out → [`cckit watch`](/copilot/) merges it.</Combo>
...
cckit plan --llm          # the session-fit wave plan, as data
...
<CardGrid>
  <LinkCard title="Driving cckit from agents" href="/agents/" description="The full machine contract for any coding agent." />
  <LinkCard title="The copilot loop" href="/copilot/" description="Fan a wave out, gate the PRs, merge the clean ones." />
  <LinkCard title="What each command prints" href="/showcase/" description="Real output for each verb." />
</CardGrid>
