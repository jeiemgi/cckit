...
## Steer a parallel session

When several sessions work on the same machine — a captain and its wave agents in one repo, or a
...
Direct messages are read-once; broadcasts are project-scoped, reach each session exactly once,
...
cckit orchestrate --runtime herdr --agent codex --cap 3 46 47 48
