...
| `cckit effort <new\|start\|pr\|close\|plan>` | The effort lifecycle. `effort new "<name>" ["sub :: desc" …]` fills the four-section plan body, applies `ctx/kind/priority/role/flow` labels, and lints every sub title — identical to `/kit-effort-new` (one shared core). Flags: `--flow/--role/--priority/--goal/--scope/--for-agents/--verification/--depends-on/--milestone`. |
