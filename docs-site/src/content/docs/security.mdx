...
1. **Copy the example** — `cp privacy-denylist.example .cckit/privacy-denylist` (the target is gitignored). Copy it into the **primary checkout's** `.cckit/`; that one file is used from every worktree.
