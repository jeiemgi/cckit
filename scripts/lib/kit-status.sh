...
status_local_rows() {
  ...
  git for-each-ref --format='%(refname:short)' refs/heads 2>/dev/null \
    | while IFS= read -r b; do
        ...
        [ -n "$effort" ] || continue
        git merge-base --is-ancestor "$b" "$effort" 2>/dev/null \
          || printf 'unmerged\t%s\tnot in %s\n' "$b" "$effort"
      done
}
...
status_prs_classify() {
  ...
  jq -r --argjson ai "${ai_expected:-0}" '
    ...
    (if   $v == "conflict"  then "merge state is CONFLICTING — rebase onto the base"
     ...
     elif $v == "wait"      then "checks still running"
    ...
    [$v, ($p.number|tostring), ($p.title // "" | .[0:58]), $d] | @tsv
  ' 2>/dev/null || true
}
...
# commit's check rollup, collapsed to the four scalars the classifier reads. best-effort — any
# failure (no gh, no jq, no auth, no network) yields `[]` rather than a broken bucket.
status_prs_fetch() {
  local repo="${1:-${KIT_REPO:-}}" owner name raw
  command -v gh >/dev/null 2>&1 || { echo '[]'; return 0; }
  command -v jq >/dev/null 2>&1 || { echo '[]'; return 0; }
  case "$repo" in */*) owner="${repo%%/*}"; name="${repo##*/}" ;; *) echo '[]'; return 0 ;; esac
...
status_cleanup_counts() {
  ...
  if command -v git >/dev/null 2>&1 && command -v gh >/dev/null 2>&1; then
    local base="${KIT_BASE_BRANCH:-main}"
    for r in $(git branch -r --format='%(refname:short)' 2>/dev/null | grep -v HEAD || true); do
      b="${r#origin/}"
      case "$b" in "$base"|main|master|develop) continue ;; esac
      st="$(gh pr list --state merged --head "$b" --limit 1 --json number --jq 'length' 2>/dev/null || echo 0)"
      case "$st" in ''|0) : ;; *) rstale=$((rstale + 1)) ;; esac
    done
  fi
