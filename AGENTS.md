...
| `cckit start <issue> [slug]` | isolated worktree + branch, bootstrapped: env files copied, per-worktree dev PORT, deps installed only where there is something to install. Precedence: **`worktree.installPaths` if set** (those dirs only — `[]` installs nothing, and the root/workspace checks are skipped entirely), **else** the root when its manifest declares dependencies or it is a workspace root **with at least one member**, **else nothing** — so a dependency-free root never gets a stray empty `pnpm-lock.yaml`. `installPaths` entries that escape the worktree are refused. `KIT_WT_INSTALL=0` skips the install | `--llm` |
