...
| `cckit start <issue> [slug]` | isolated worktree + branch, bootstrapped: env files copied, per-worktree dev PORT, deps installed only where the project declares any (root manifest with deps / workspace root, else `worktree.installPaths`, else nothing — so a dependency-free root never gets a stray empty `pnpm-lock.yaml`). `KIT_WT_INSTALL=0` skips the install | `--llm` |
